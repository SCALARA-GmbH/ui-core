...
import { CONTENT_VARIANTS, TEXT_COLORS, TITLE_VARIANTS, VARIANTS } from './types';
...
export const TypographyStorySBS = (): JSX.Element => {
  ...
  return (
    <Grid xs={1} style={{ backgroundColor: theme.colors.background.primary, padding: 16 }}>
      {TITLE_VARIANTS.map((variant) => (
        <Typography key={variant} variant={variant}>
          {variant.toUpperCase()} Desktop {getFontSize(variant)}px {getFontWeight(variant)} weight. <br />
          With SCALARA you can concentrate on the essentials.
        </Typography>
      ))}
      {CONTENT_VARIANTS.map((variant) => (
        <Typography key={variant} variant={variant}>
          {variant.toUpperCase()} Desktop {getFontSize(variant)}px {getFontWeight(variant)} weight
          <br />
          The platform enables efficient, smooth and transparent communication between managers, owners, tenants and
          even service providers. With SCALARA you can concentrate on the essentials.{' '}
          <a href={'https://scalara.de/'} target="_blank" rel="noreferrer">
            SCALARA
          </a>{' '}
          Our highest requirement is to develop a technically correct and legally compliant software.
        </Typography>
      ))}
    </Grid>
  );
    ...
    <Grid xs={1} style={{ backgroundColor: theme.colors.background.primary, padding: 16 }}>
      ...
      {CONTENT_VARIANTS.map((v) => (
        <Typography key={v} disabled variant={v}>
          ...
          tortor urna. Ut laoreet sodales nisi, quis iaculis nulla iaculis vitae. Donec.
        </Typography>
      ))}
    </Grid>
  );
...
export const TypographyUnderlinedStory = (): JSX.Element => {
  ...
  return (
    <div style={{ backgroundColor: theme.colors.background.primary, padding: 16 }}>
      <Switch label={'underlined'} checked={underlined} onChange={(event) => setUnderlined(event.target.checked)} />
      <Switch
      ...
      <Grid xs={2} style={{ padding: 16 }}>
        {TEXT_COLORS.map((c) => (
          <GridItem key={c}>
            <Typography color={c} underlined={underlined} underlinedAnimated={underlinedAnimated}>
              {c}
            </Typography>
          </GridItem>
        ))}
    ...
    <Grid xs={2} style={{ backgroundColor: theme.colors.background.primary, padding: 16 }}>
      {TEXT_COLORS.map((c) => (
        <Typography key={c} color={c} gutterBottom>
          ...
          iaculis nulla iaculis vitae. Donec.
        </Typography>
      ))}
    </Grid>
  );
...
export const LabStory = (): JSX.Element => (
  <div>
    <Typography
      ...
      wrap={select('wrap', [Wrap.NO_WRAP, Wrap.PRESERVE_BREAK_LINES, Wrap.DEFAULT], Wrap.DEFAULT)}
    >
      {text(
        'Text',
